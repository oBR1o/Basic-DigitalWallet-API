...
async def test_create_merchants(client: AsyncClient, token_user1: Token):
    ...
    payload = {"name": "merchants", "user_id": str(token_user1.user_id)}
    ...
    assert data["id"] > 0
    assert str(data["user_id"]) == str(token_user1.user_id)

@pytest.mark.asyncio
async def test_update_merchants(
    client: AsyncClient, merchant_user1: models.DBMerchant, token_user1: Token
):
    headers = {"Authorization": f"{token_user1.token_type} {token_user1.access_token}"}
